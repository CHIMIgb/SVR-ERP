...
describe('ConciliacionBancariaService', () => {
  ...
  describe('cargarLote', () => {
    ...
    it('debe reportar el número de LÍNEA FÍSICA del archivo aunque haya líneas vacías (Blocker menor #1)', async () => {
      const csv =
        '2026-02-31,Febrero imposible,100,\n' + // línea 1: FECHA_INVALIDA
        '\n' + // línea 2: vacía (no se numera, no se descarta)
        '2026-04-30,Abril válido,,300\n' + // línea 3: válida
        '\n' + // línea 4: vacía
        'basura,sin monto,,\n'; // línea 5: MONTO_INVALIDO
      prisma.movimientos_bancarios.createMany.mockResolvedValue({ count: 1 });
      const result = await service.cargarLote(CUENTA_ID, { csv }, USER_ID);
      expect(result).toEqual({
        totalMovimientos: 1,
