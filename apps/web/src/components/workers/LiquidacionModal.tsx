...
import { X, Printer, UserX, Scale, Loader2 } from 'lucide-react';
...
export default function LiquidacionModal({
  ...
  const [selectedWorkerId, setSelectedWorkerId] = useState<string>(initialTrabajador?.id ?? activos[0]?.id ?? '');
  const worker = trabajadores.find((t) => t.id === selectedWorkerId) ?? initialTrabajador ?? activos[0];
  ...
  const handleEjecutarBaja = async () => {
    ...
    if (res.success) {
      showToast(`⚖️ Baja y Liquidación de ${worker.nombre} registrada por ${fmt.format(res.data.granTotalNeto)}.`, 'success');
      addNotification({
      ...
      <Overlay onClick={onClose} blur className="z-[9999] p-4">
        ...
        >
          ...
          <div className="px-6 py-4 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 text-white flex items-center justify-between border-b border-slate-800">
            ...
            <div className="flex items-center gap-2">
              <div className="flex bg-slate-800 p-1 rounded-xl">
                <button onClick={() => setViewMode('calculadora')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'calculadora' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>
                  Calculadora
                </button>
                <button onClick={() => setViewMode('carta_firma')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase transition-all ${viewMode === 'carta_firma' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>
                  🖨️ Carta con Firma
                </button>
              </div>
              <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white/70 hover:text-white flex items-center justify-center transition-colors ml-2">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            {viewMode === 'carta_firma' && (
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-md text-slate-900 space-y-6 font-serif">
                ...
                <div className="pt-4 flex justify-end gap-2 font-sans border-t border-slate-100">
                  <button onClick={handlePrint} className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="px-6 py-4 bg-slate-900 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-slate-800">
            ...
            <div className="flex items-center gap-2">
              <button onClick={onClose} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold transition-colors">Cancelar</button>
              <button
                onClick={handleEjecutarBaja}
                disabled={submitting}
                className="px-5 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all shadow-lg shadow-red-600/30"
              >
